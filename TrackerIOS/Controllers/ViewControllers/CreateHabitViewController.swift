...
final class CreateHabitViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            tableViewForChooseCategoryOrSchedule.heightAnchor.constraint(equalToConstant: 150),
...
//MARK: Actions
    @objc private func createButtonTapped(_ sender: UIButton) {
        ...
        guard let color = UIColor(named: selectedColor) else {print("Цвета не существует"); return}
                ...
                let newTask = TrackerCategory(header: selectedCategory, trackers: [Tracker(id: UUID(), name: name, colorName: color, emoji: selectedEmoji, schedule: selectedSchedule)])
...
extension CreateHabitViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return categoriesOrSchedule.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == categoriesOrSchedule.count - 1 {
...
extension CreateHabitViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if arr == "Категория" {
            ...
            let navigationSchedule = ScheduleViewConroller()
            let navigation = UINavigationController(rootViewController: navigationSchedule)
            navigationSchedule.scheduleToPass = { [weak self] schedule in
                guard let self = self,
                      let cell = tableView.cellForRow(at: indexPath) else { return }
